...
Local development runs the Next.js app against deployed Cloudflare Workers. Set `PROBE_WORKERS_SUBDOMAIN` and `PROBE_SECRET` in `.env.local`.
...
- Regional Workers path: `probes/cloudflare/` and `npm run probe:cf:dev` / deploy scripts.
