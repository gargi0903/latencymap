...
4. `runRegionalTest` fans out with `Promise.all` to configured probes:
   ...
   - **Local dev:** one Node probe at `http://127.0.0.1:8787/probe` (not regional latency)
...
5. Each probe (`probes/cloudflare/src/worker.ts` or `probes/node/server.ts`):
...
Local Node probe returns `placement_region: null` and `cloudflare_colo: null`.
...
```bash
npm install
npm run dev:local    # Next.js + local Node probe together
```

Local mode uses **one** probe region (`local`) — useful for flow testing, not real regional latency.
